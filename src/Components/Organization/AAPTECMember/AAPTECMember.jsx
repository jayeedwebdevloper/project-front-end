    ...
    return (
        <div className='container mx-auto px-4 lg:px-6'>
            <div className="pt-40 pb-8">
                <h1 className='text-center text-slate-700 text-2xl pb-8 font-light'>AAPTEC Member</h1>
                <div className="grid gap-8 grid-cols-1 md:grid-cols-2 lg:grid-cols-3 w-11/12 mx-auto xl:px-10">
                    {
                        apptecMember.map((member, i) => <div className='shadow-md border flex flex-col items-center justify-center p-5' key={i}>
                            <img className='h-40 w-40 rounded-full' src={member.photo} alt={member.name} />
                            <h4 className='text-slate-800 text-lg font-semibold pt-4 pb-2'>{member.name}</h4>
