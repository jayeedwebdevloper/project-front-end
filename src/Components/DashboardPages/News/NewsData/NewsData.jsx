...
const NewsData = () => {
    ...
    const addNews = (e) => {
        e.preventDefault();
        const newNews = e.target.newNews.value;
        const newsTitle = e.target.newsTitle.value;
        const date = nowTime;
        ...
        fetch('https://api.cloudinary.com/v1_1/dnhefldpl/image/upload', {
            ...
            .then(data => {
                if (data.url) {
                    ...
                    fetch('https://back-end-bice-eight.vercel.app/add-news', {
                        method: "POST",
                        headers: {
    ...
    const deleteNews = (id) => {
        const confirm = window.confirm("Are You Sure to Delete That News ?");
        if (confirm) {
            fetch(`https://back-end-bice-eight.vercel.app/delete-news/${id}`, {
                method: "DELETE"
            })
                .then(res => res.json())
                .then(data => {
    ...
    return (
        <div className='container mx-auto px-5 lg:px-10'>
            <div className="py-5">
                <h1 className='text-xl text-slate-600 font-semibold'>News</h1>
                <div className="py-4">
                    <div className="overflow-x-auto border rounded-lg shadow mb-7">
                        <table className="table table-xs table-pin-rows table-pin-cols overflow-hidden border-t rounded-lg">
                            <thead>
                                <tr className='py-5'>
                                    <td className='bg-primary/90 text-white text-[17px] font-normal py-3 px-8'>Photo</td>
                                    <td className='bg-primary/90 text-white text-[17px] font-normal py-3 px-8'>Title</td>
                                    <td className='bg-primary/90 text-white text-[17px] font-normal py-3 px-8'>News</td>
                                    <td className='bg-primary/90 text-white text-[17px] font-normal py-3'>Added Date</td>
                                    <td className='bg-primary/90 text-white text-[17px] font-normal py-3 px-8'>
                                        Action
                                    </td>
                                </tr>
                            </thead>
                            <tbody>
                                {
                                    news.map((data, i) => <tr key={i}>
                                        <td className='py-1 text-[13px] px-5'><img className='h-12' src={data.photo} alt={data.title} /></td>
                                        <td className='py-1 text-[13px]'>{data.newsTitle}</td>
                                        <td className='py-1 text-[13px]'>{data.newNews}</td>
                                        <td className='py-1 text-[13px]'>{data.date}</td>
                                        <td className='py-1 text-[13px] px-5 text-end'> <button onClick={() => deleteNews(data._id)} className='btn btn-sm btn-ghost btn-circle'><svg xmlns="http://www.w3.org/2000/svg" height="16" width="14" className='fill-red-800' viewBox="0 0 448 512"><path d="M170.5 51.6L151.5 80h145l-19-28.4c-1.5-2.2-4-3.6-6.7-3.6H177.1c-2.7 0-5.2 1.3-6.7 3.6zm147-26.6L354.2 80H368h48 8c13.3 0 24 10.7 24 24s-10.7 24-24 24h-8V432c0 44.2-35.8 80-80 80H112c-44.2 0-80-35.8-80-80V128H24c-13.3 0-24-10.7-24-24S10.7 80 24 80h8H80 93.8l36.7-55.1C140.9 9.4 158.4 0 177.1 0h93.7c18.7 0 36.2 9.4 46.6 24.9zM80 128V432c0 17.7 14.3 32 32 32H336c17.7 0 32-14.3 32-32V128H80zm80 64V400c0 8.8-7.2 16-16 16s-16-7.2-16-16V192c0-8.8 7.2-16 16-16s16 7.2 16 16zm80 0V400c0 8.8-7.2 16-16 16s-16-7.2-16-16V192c0-8.8 7.2-16 16-16s16 7.2 16 16zm80 0V400c0 8.8-7.2 16-16 16s-16-7.2-16-16V192c0-8.8 7.2-16 16-16s16 7.2 16 16z" /></svg></button></td>
                                    </tr>).reverse()
                                }
                            </tbody>
                        </table>
                    </div>
                    <button onClick={() => document.getElementById('news').showModal()} className='btn btn-warning rounded-none font-thin text-white tracking-widest h-4'>Add News</button>
                    <dialog id="news" className="modal">
                        <div className="modal-box">
                            <form method="dialog">
                                {/* if there is a button in form, it will close the modal */}
                                <button id='close' className="btn btn-sm btn-circle btn-ghost absolute right-2 top-2">✕</button>
                            </form>
                            <h3 className="font-bold text-lg">Add News</h3>
                            <form onSubmit={addNews} className='form-control'>
                                <div className="py-2">
                                    <label htmlFor="newsTitle" className='label'>News Title</label>
                                    <input type="text" id='newsTitle' name='newsTitle' className='input input-bordered h-10 rounded-none mb-2 w-full' />

                                    <div className="pb-3">
                                        <label htmlFor="image" className='label'>News Image <sub>( Image Size Must Be Wide )</sub></label>
                                        <div className="border border-dashed border-slate-400 rounded-md flex flex-col items-center justify-center overflow-hidden h-54 p-1" onClick={() => document.querySelector('.photo').click()}>
                                            ...
                                            }} type="file" name="doctor-image" id="image" draggable='true' className='photo' hidden />
                                            {
                                                avatar ? <img className='w-full h-full rounded-md' src={preview} alt='Doctor' /> : <>
                                                    <p className='text-slate-400'>Upload News Photo</p>
                                    ...
                                    <label htmlFor="newNews" className='label'>News Details</label>
                                    <textarea name="newNews" id="newNews" rows="3" className='textarea textarea-bordered rounded-none w-full'></textarea>
                                </div>
