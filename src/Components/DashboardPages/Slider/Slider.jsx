...
const Slider = () => {
            ...
            .then(data => {
                if (data.url) {
                    const slider = {
                        ...
                        caption,
                        link,
                        sliderImage: data.url
                    }

                    fetch('https://back-end-bice-eight.vercel.app/add-slide', {
                        method: 'POST',
                        headers: {
    ...
    const deleteSlider = (id) => {
        ...
        if (confirm) {
            fetch(`https://back-end-bice-eight.vercel.app/delete-slide/${id}`, {
                method: "DELETE"
            })
                .then(res => res.json())
                .then(data => {
    ...
    return (
        <div className='container mx-auto px-5 lg:px-10'>
            <div className="py-4">
                <h1 className='text-xl text-slate-700 font-semibold'>Slider</h1>
                <div className="py-4">
                        ...
                        <table className="table table-xs table-pin-rows table-pin-cols overflow-hidden border-t rounded-lg">
                            <thead>
                                <tr className='py-5'>
                                    ...
                                    <td className='bg-primary/90 text-white text-[17px] font-normal py-3'>Caption</td>
                                    <td className='bg-primary/90 text-white text-[17px] font-normal py-3'>Button Link</td>
                            ...
                            <tbody>
                                {
                                    sliders.map((slider, i) => <tr key={i}>
                                        <td className='py-2 text-[13px] px-5'>
                                            <img className='h-20 rounded-md' src={slider?.sliderImage} alt={slider?.title} />
                                        </td>
                                        <td className='py-2 text-[14px] capitalize'>{slider?.title}</td>
                                        <td className='py-2 text-[14px] capitalize'>{slider?.caption}</td>
                                        <td className='py-2 text-[14px]'>{slider?.link}</td>
                                        <td className='py-2 text-[13px] px-5 text-end'> <button className='btn btn-sm btn-ghost btn-circle' onClick={() => deleteSlider(slider._id)}><svg xmlns="http://www.w3.org/2000/svg" height="16" width="14" className='fill-red-800' viewBox="0 0 448 512"><path d="M170.5 51.6L151.5 80h145l-19-28.4c-1.5-2.2-4-3.6-6.7-3.6H177.1c-2.7 0-5.2 1.3-6.7 3.6zm147-26.6L354.2 80H368h48 8c13.3 0 24 10.7 24 24s-10.7 24-24 24h-8V432c0 44.2-35.8 80-80 80H112c-44.2 0-80-35.8-80-80V128H24c-13.3 0-24-10.7-24-24S10.7 80 24 80h8H80 93.8l36.7-55.1C140.9 9.4 158.4 0 177.1 0h93.7c18.7 0 36.2 9.4 46.6 24.9zM80 128V432c0 17.7 14.3 32 32 32H336c17.7 0 32-14.3 32-32V128H80zm80 64V400c0 8.8-7.2 16-16 16s-16-7.2-16-16V192c0-8.8 7.2-16 16-16s16 7.2 16 16zm80 0V400c0 8.8-7.2 16-16 16s-16-7.2-16-16V192c0-8.8 7.2-16 16-16s16 7.2 16 16zm80 0V400c0 8.8-7.2 16-16 16s-16-7.2-16-16V192c0-8.8 7.2-16 16-16s16 7.2 16 16z" /></svg></button></td>
                                    </tr>).reverse()
                                }
                            </tbody>
                        </table>
                    </div>
                    <button onClick={() => document.getElementById('slide').showModal()} className='btn btn-warning rounded-none font-thin text-white tracking-widest h-4'>Add Slide</button>
                    <dialog id="slide" className="modal">
                        <div className="modal-box">
                            ...
                            <h3 className="font-bold text-lg">Add Slide</h3>
                            <form onSubmit={handleSubmit(addSlider)} className='form-control'>
                                <div className="py-2">
                                ...
                                <div className="py-2">
                                    ...
                                    <input id='caption' name='caption' {...register("caption", { required: true })} className='input input-bordered rounded-none w-full h-10' type="text" />
                                </div>
                                <div className="py-2">
                                ...
                                <div className="py-2">
                                    <label htmlFor="image" className='label'>Slider Image <sub>( Image Size Must Be Wide )</sub></label>
